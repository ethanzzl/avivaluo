...
- 中文为默认语言，英文使用 `/en/` 路径。
- 当前包含首页、作品列表、8 个作品详情、关于、联系、隐私和 404。
- 生产站点当前发布 8 个项目或合集，共引用 64 张经过保护处理的网页图片。
...
2. 识别重复内容并保留母版。
3. 按商业应用而不是单纯文件名分组。
4. 首轮将 57 个可公开素材整理为 7 个项目或合集，并精选 39 张进入网站。
5. 后续整理公开的 Instagram 与小红书作品，把项目扩充为当前的 8 个合集、64 张网页图片引用；其余素材继续保留为候选。
...
| `app/site.tsx` | 首页、作品、项目详情、关于、联系与隐私内容 |
| `app/site-shell.tsx` | 中英文共享的导航、页脚、页面框架与 404 内容 |
| `app/(zh)/` | 中文静态页面与 `lang="zh-CN"` 根布局 |
| `app/en/` | 英文静态页面与 `lang="en"` 根布局 |
| `app/site-config.ts` | 唯一正式域名与内容更新时间 |
| `app/site-metadata.ts` | canonical、hreflang、Open Graph 与页面 metadata |
| `app/robots.ts`、`app/sitemap.ts` | 固定正式域名的搜索引擎入口 |
...
公开网站只使用 `public/images/projects/protected/` 中的网页衍生图：最长边限制为 1800px，使用适合网页浏览的 WebP 压缩，并写入作者与版权 EXIF 信息。较大的来源文件保存在 `source-assets/projects/`，不通过网站公开 URL 提供。

页面同时采用以下保护措施：

- 作品图关闭浏览器原生拖拽。
- 项目详情页显示中英文版权与禁止商业使用提示。
- 旧的大尺寸图片公开路径返回 404。
- 不屏蔽右键，以免破坏正常浏览与可访问性。
- 不添加明显水印，以免影响作品集的专业呈现。

这些措施用于降低未经授权直接取用的便利度，不能完全阻止截图或通过开发者工具保存。替换或增加作品时，先将来源图放入 `source-assets/projects/`，再运行 `npm run images:protect` 生成公开衍生图；不要直接把母版放入 `public/`。
...
- 中文和英文现在使用独立根布局，英文页面可被正确识别为 `lang="en"`。
- canonical、robots、sitemap 和 Open Graph 使用唯一正式域名，不会被预览 Host 污染。
- 旧 Cloudflare、Vite、vinext、D1、Drizzle 与未使用认证代码已经移除。
- 项目发布状态、版权确认、内容验证、GitHub Actions 与独立项目分享图已经建立。
...
5. 补充真实结构化数据。
   每个项目已经使用自己的封面作为 Open Graph 图片；后续可根据页面真实内容补充 `Person`、`CreativeWork` 或 `VisualArtwork` 结构化数据。

#### P1：技术与维护

6. 继续优化图片交付。
   当前 69 张公开保护版 WebP 总量约 8.2 MB，最长边不超过 1800px；后续可根据真实显示宽度生成多档响应式图片，并评估 AVIF，进一步降低移动端流量。
...
- [ ] 没有裁掉主体、签名或作品文字
- [ ] 图片清晰但不是未压缩母版
- [ ] 新图片已通过 `npm run images:protect` 生成公开衍生图
- [ ] 页面未引用 `source-assets/` 或旧的大尺寸公开路径
